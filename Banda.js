...
class Banda {
	...
	generarPuntos() {
		...
		let centroAmplitud = this.grosor * random(0.02, 0.06);
		...
		for (let x = this.inicioX; x <= this.finX; x += step) {
			...
			let largoBanda = this.finX - this.inicioX;
			let largoFade = largoBanda * 0.25;

			// afina solo si la punta NO toca el borde izquierdo
			if (this.inicioX > 5 && x < this.inicioX + largoFade) {
				fadeExtremos = map(x, this.inicioX, this.inicioX + largoFade, 0.08, 1);
			}

			// afina solo si la punta NO toca el borde derecho
			if (this.finX < width - 5 && x > this.finX - largoFade) {
				let fadeOut = map(x, this.finX - largoFade, this.finX, 1, 0.06);
				fadeExtremos = min(fadeExtremos, fadeOut);
			}

			fadeExtremos = constrain(fadeExtremos, 0.06, 1);

			let grosorActual = this.grosor * factorGrosor * fadeExtremos;
	...
	generarFragmentoIzquierdo() {
		...
		for (let x = inicioXIzq; x <= finXIzq; x += step) {
			...
			let fadeExtremos = 1;
			let largoFragmento = finXIzq - inicioXIzq;
			let largoFade = largoFragmento * 0.6;

			// no afinamos contra el borde 0, solo hacia la punta interior
			if (x > finXIzq - largoFade) {
				let fadeOut = map(x, finXIzq - largoFade, finXIzq, 1, 0.10);
				fadeExtremos = min(fadeExtremos, fadeOut);
			}

			fadeExtremos = constrain(fadeExtremos, 0.09, 1);
	...
	generarFragmentoDerecho() {
		...
		for (let x = inicioXDer; x <= finXDer; x += step) {
			...
			factorGrosor = constrain(factorGrosor, 0.5, 1.5);
			let fadeExtremos = 1;
			let largoFragmento = finXDer - inicioXDer;
			let largoFade = largoFragmento * 0.6;

			// no afinamos contra el borde width, solo hacia la punta interior
			if (x < inicioXDer + largoFade) {
				let fadeIn = map(x, inicioXDer, inicioXDer + largoFade, 0.10, 1);
				fadeExtremos = min(fadeExtremos, fadeIn);
			}

			fadeExtremos = constrain(fadeExtremos, 0.10, 1);
	...
	dibujarFragmentada() {
		...
		for (let seg of segmentos) {
			...
			// Afinar extremos de cada fragmento para evitar cortes rectos
			let largoSegmento = endX - startX;
			let largoFade = largoSegmento * 0.35;

			for (let i = 0; i < puntosSupSeg.length; i++) {
				let pSup = puntosSupSeg[i];
				let pInf = puntosInfSeg[i];

				let centroY = (pSup.y + pInf.y) / 2;
				let mitadGrosor = (pInf.y - pSup.y) / 2;

				let fade = 1;

				if (pSup.x < startX + largoFade) {
					fade = map(pSup.x, startX, startX + largoFade, 0.05, 1);
				}

				if (pSup.x > endX - largoFade) {
					let fadeOut = map(pSup.x, endX - largoFade, endX, 1, 0.05);
					fade = min(fade, fadeOut);
				}

				fade = constrain(fade, 0.05, 1);

				pSup.y = centroY - mitadGrosor * fade;
				pInf.y = centroY + mitadGrosor * fade;
			}
